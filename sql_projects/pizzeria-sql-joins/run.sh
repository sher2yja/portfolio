...
DB_NAME="${DB_NAME:-sqlb3_joins}"
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Названия из условий дня: меню без них показывает голые номера.
TITLES=(
  "Пиццерии, где никто не был"
  "Пропущенные даты визитов"
  "FULL JOIN: обе стороны целиком"
  "То же самое через CTE"
  "Цены на грибную и пепперони"
  "Женщины старше 25 лет"
  "Что заказывали Денис и Анна"
  "Где Дмитрий мог поесть дешевле 800"
  "Мужчины Москвы и Самары"
  "И пепперони, и сырная сразу"
  "Соседи по городу"
)
LAST=$(( ${#TITLES[@]} - 1 ))
...
show_menu() {
    echo "Соединения таблиц и CTE  (база: $DB_NAME)"
    echo
    for i in "${!TITLES[@]}"; do
        printf "  %2d  ex%02d  %s\n" "$i" "$i" "${TITLES[$i]}"
    done
