...
[n8n в локальном кластере k3s](./n8n-k3s-local) — VM с Ansible, три роли n8n, Helm, Prometheus и Grafana; восстановление credential проверено. Реальный CI и полное развёртывание из свежего клона ещё не подтверждены, поэтому проект пока не включён в счёт тринадцати опубликованных.
