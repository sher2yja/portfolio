...
Локальный стенд n8n в режиме очередей: отдельные main, webhook и worker, PostgreSQL и Redis. Виртуальная машина создаётся Vagrant, настраивается Ansible; отдельный host-playbook Ansible устанавливает таймеры бэкапов без их включения и подключает мониторинг после создания окружений. Приложение разворачивается манифестами Kubernetes или собственным Helm-чартом через GitHub Actions. В проекте также проверены мониторинг, обновление и откат релиза, резервное копирование и восстановление сохранённых credentials.
...
Для staging и production созданы разные секреты из `.env` и `.env.production`; в репозиторий они не входят. Для восстановления credentials нужны **и дамп базы, и исходный ключ шифрования**. Ansible содержит ежедневный пользовательский systemd-таймер для production-копии и еженедельный для контрольного восстановления, но оба оставлены выключенными по решению владельца. Сервисы можно запускать вручную; сохраняются последние 14 проверенных копий. Отдельный снимок production-копии зашифрован GPG и проверен расшифровкой и SHA-256 на USB-носителе. Автоматическое обновление копии на USB не настроено; локальные копии по-прежнему не зашифрованы отдельно от учётной записи хоста.

После настройки VM, обоих namespace и локальных Secret запустите на хосте `ansible-playbook -i localhost, ansible/host.yml`. Он устанавливает unit-файлы таймеров без включения и подключает мониторинг; приложение по-прежнему доставляет GitHub Actions. При повторном прогоне host-playbook должен показать `changed=0`.
