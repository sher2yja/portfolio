...
Для staging и production созданы разные секреты из `.env` и `.env.production`; в репозиторий они не входят. Для восстановления credentials нужны **и дамп базы, и исходный ключ шифрования**. Резервные копии остаются на том же компьютере: внешнее независимое хранилище здесь не проверено.
