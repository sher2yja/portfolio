...
VM использует libvirt, Ubuntu 24.04 и k3s `v1.35.8+k3s1`. Повторный Ansible-прогон на проверенном стенде завершился с `changed=0`. Позже новая VM была создана из отобранной папки портфолио; Helm поднял пять компонентов, `/healthz` ответил `{"status":"ok"}`. Собственный образ импортирован в k3s без внешнего реестра и применён ко всем трём ролям n8n. Проверка из свежего публичного Git-клона ещё не выполнялась: публикации пока нет.
