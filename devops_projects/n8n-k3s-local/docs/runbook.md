...
`workflows/cluster-summary.json` и `workflows/ci-failure-watch.json` первоначально импортированы в production неактивными. Затем в интерфейсе n8n для них выбран существующий Telegram credential, `SET_CHAT_ID_IN_N8N` заменён числовым ID чата, проверен часовой пояс расписания сводки (09:00), выполнены ручные тесты и оба workflow активированы. База production подтверждает `active=true` для них и `DO14 Telegram alerts`; у CI-наблюдателя есть успешные исполнения по расписанию, а ручной запуск сводки доставил сообщение. Сводка показывает перезапуски Pod за последние 24 часа, неготовые Pod и занятость диска. CI-наблюдатель при первом штатном запуске запоминает существующие ошибки без отправки старых уведомлений; после новой неуспешной GitHub Actions run отправляет ссылку. Токен и chat ID не вписывайте в файлы репозитория.
...
В `do14-restore-credential` выполнены дополнительные изолированные опыты. При другом `N8N_ENCRYPTION_KEY` только у worker запрос к workflow с PostgreSQL credential не завершился за 90 секунд, а после возврата общего ключа вновь вернул HTTP 200 и `{"credential_ok":1}`; прямую ошибку расшифровки ещё нужно установить. Одиночный запрос к `do14-smoke` при остановленном Redis дал `fetch failed`, после восстановления Redis — HTTP 200. Затем при десяти параллельных 30-секундных `do14-load` задачах Redis остановлен через пять секунд. Клиентское исполнение оборвалось с кодом 137, тестовые роли перезапустились. После возврата Redis новые задачи снова завершались с HTTP 200, но через две минуты из десяти прерванных execution пять числились `crashed` и пять `running`. Считать их успешно повторёнными нельзя. Staging и production не затронуты, тестовые компоненты возвращены к `0/0`.

Для проверки Helm-антипаттерна во временную копию чарта добавлен Secret, генерирующий ключ через `randAlphaNum` при upgrade. Применялся только `do14-restore-credential`; версия n8n, пароль PostgreSQL и токен runner остались прежними. Main упал на чтении сохранённого `signing.hmac` с сообщением `Deployment key 'signing.hmac' cannot be read with this instance encryption key`. `--atomic --wait` после тайм-аута откатил первый upgrade. При повторном коротком upgrade ошибка зафиксирована в журнале, затем `helm rollback` восстановил штатную ревизию; тест credential снова вернул HTTP 200 и `{"credential_ok":1}`. Временный Secret отсутствует, тестовый чарт удалён, namespace масштабирован до нуля. Именно поэтому рабочий чарт требует существующий Secret вместо генерации ключа при рендеринге.
...
На первом этапе на единственном доступном ПК были включены пользовательские таймеры: ежедневный `do14-backup.timer` и воскресный `do14-backup-verify.timer`. Первый ручной запуск `do14-backup.service` создал production-копию `~/.local/share/do14-backups/production/20260925T151506Z`; `sha256sum -c` подтвердил дамп, ключ и Helm values. Каталог имеет права `0700`. Ручной запуск `do14-backup-verify.service` восстановил этот дамп в одноразовый PostgreSQL 17.6 без опубликованных портов и нашёл 142 таблицы и 2 credentials. Проверка не расшифровывает credentials ключом n8n; это отдельно подтверждено прежним восстановлением приложения. Локальные копии остаются на одном ПК и не зашифрованы отдельно от его учётной записи.

Владелец согласовал host-таймер как замену первоначально запланированному еженедельному workflow внутри n8n: проверка должна работать и при недоступности самого n8n. Восстановление дампа и контрольные запросы выполняет таймер, а не сценарий n8n; это различие сохраняется в отчёте. После настройки Telegram создана production-копия `20260925T170609Z`, и `sha256sum -c` подтвердил все три файла. Позднее создана и проверена новая копия `20260925T181958Z`.

Проверка запуска через systemd без ожидания ночи: два одноразовых пользовательских таймера через 5 и 8 минут вызвали штатные `do14-backup.service` и `do14-backup-verify.service`. Первый создал копию `20260925T185144Z` с тремя совпавшими SHA-256. Второй восстановил именно её во временный PostgreSQL: 142 таблицы, 2 credentials, оба сервиса завершились успешно. Постоянные таймеры на 03:00 ежедневно и воскресенье 04:00 остались активными и неизменёнными; их первый календарный запуск ещё не наблюдался.

После опыта владелец попросил удалить все таймеры. Два разовых уже завершились и исчезли; два постоянных остановлены и отключены, их unit-файлы удалены из проекта и Ansible больше их не устанавливает. Автоматических копий и проверок теперь **нет**. Для ручного запуска после `ansible/host.yml` используйте `systemctl --user start do14-backup.service`, затем `systemctl --user start do14-backup-verify.service`. Существующие локальные и USB-копии сохранены.

Копия `20260925T154851Z` вручную зашифрована GPG AES-256 в `DO14-backups/production-20260925T154851Z.tar.gpg` на USB-носителе. Исходные файлы до экспорта прошли `sha256sum -c`. Проверка USB-архива потребовала парольную фразу, показала четыре ожидаемых файла и распаковала их во временный каталог `0700`; там все три контрольные суммы совпали. Открытые временные файлы затем удалены. Более свежая копия `20260925T181958Z` зашифрована в отдельный USB-архив; GPG распознал AES-256, файл записан и синхронизирован, владелец повторно проверил расшифровку и получил `OK` для дампа, `.env` и values. Парольную фразу нужно хранить отдельно от ПК и флешки: без неё архив не восстановить. USB-снимки пока обновляются вручную.
...
- Несовпадающий ключ main/worker: установить точную причину тайм-аута на workflow с сохранёнными credentials, не публикуя значения ключей.
...
- Остановка Redis: выяснить судьбу зависших execution и проверить восстановление очереди после отказа под нагрузкой.
