...
Prometheus собирал метрики main и webhook: обе цели отображались как `up`. Проверено наличие метрик очереди, в том числе `n8n_scaling_mode_queue_jobs_waiting` и `n8n_scaling_mode_queue_jobs_active`. Grafana загрузила локальный дашборд. Четыре правила алертов прошли `promtool check rules`. При остановке только staging main метрика `up{job="n8n-main"}` стала нулём, `N8nMainUnavailable` последовательно перешёл из `pending` в `firing`. После восстановления Deployment main ответил `/healthz`, метрика вернулась к единице, алерт исчез. Production при этом оставался доступен. Хранилище мониторинга — `emptyDir`; долговременное хранение не испытывалось.
...
- отказ worker или Redis под нагрузкой; повторная отправка Telegram после ротации токена;
