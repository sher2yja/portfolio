...
#
# Исключения применяются только к найденным подкаталогам. Сам TARGET_ROOT
# добавляется после фильтрации и не проверяется: он выбран пользователем
# осознанно, и вычёркивать его нельзя.
#
# Раньше фильтр применялся и к корню тоже. Из-за этого при корне внутри
# /tmp список кандидатов оказывался пустым, цикл завершался на первом же
# шаге, и скрипт сообщал об успешном завершении, не создав ничего.
get_valid_dirs() {
  local found
  found=$(find "$TARGET_ROOT" -mindepth 1 -maxdepth 2 -type d 2>/dev/null \
            | grep -v -E "/(bin|sbin|media)(/|$)")

  # /tmp исключается только тогда, когда корень работы лежит вне его.
  # Смысл исключения — не сорить во временном каталоге при обходе от корня
  # файловой системы. Если пользователь сам выбрал корнем каталог внутри
  # /tmp, вычёркивать его содержимое неправильно: тогда единственным
  # кандидатом остаётся сам корень и разбрасывание по подкаталогам пропадает.
  case "$TARGET_ROOT" in
    /tmp|/tmp/*) ;;
    *) found=$(printf '%s\n' "$found" | grep -v "^/tmp") ;;
  esac

  # grep -v '^$' убирает пустую строку, которую printf выдаёт при пустом
  # списке найденного, — иначе она стала бы кандидатом с пустым путём
  { echo "$TARGET_ROOT"; printf '%s\n' "$found"; } | grep -v '^$' | head -500
}
