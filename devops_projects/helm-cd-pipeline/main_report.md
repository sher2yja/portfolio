...
Стадии три, а файлов workflow два, и в прогоне по ветке видно только две
джобы. Расхождение стоит объяснить сразу, потому что оно не случайное:

```
helm-cd-pipeline.yml    build ──► test      push в feature_* + paths
helm-cd-staging.yml     staging             push тега helm-cd-v*
```

Собрать всё в один файл не даёт устройство самого GitHub Actions, причём
сразу с трёх сторон.

**`paths:` и `tags:` не уживаются в одном блоке `on.push`.** Фильтр по путям
работает через сравнение двух состояний. При пуше тега пушится новый ref,
и сравнивать не с чем — фильтр перестаёт быть предсказуемым. А `paths:`
здесь обязателен: репозиторий общий на четырнадцать проектов, без него
конвейер этого запускался бы на правку любого другого.

Сам фильтр при этом уже, чем каталог проекта: из него вычтены `**.md`
и `images/**`. Отчёт и скриншоты правятся чаще всего остального, а
поднимать ради них одноразовый кластер незачем. Отрицания стоят после
положительного образца, потому что порядок значим, и правка чарта вместе
с отчётом прогон всё равно запускает — файл чарта остаётся включённым.

Обратный случай: под фильтр заведены **оба** файла workflow, хотя запускает
он только один. Причина в том, что `actionlint` в стадии `build` проверяет
их вместе, а релизный файл исполняется исключительно по тегу — ошибка
в нём всплыла бы уже во время выпуска. Именно так и вышло с первым прогоном
`staging`: он упал на разборе файла, не дойдя до создания джобы.

**`permissions` задаются на файл, а не на джобу.** Конвейеру проверок
хватает `contents: read`. Стадии `staging` нужны `contents: write`, чтобы
приложить `.tgz` к релизу, и `packages: write` для `helm push` в реестр.
В одном файле эти права получил бы каждый прогон по любой ветке `feature_*`
— то есть ровно то разделение, ради которого ветка и тег разведены,
исчезло бы на уровне прав.

**`concurrency` тоже задаётся на файл.** У конвейера проверок стоит
`cancel-in-progress: true`: новый пуш в ветку гасит предыдущий прогон,
и это правильно — устаревшая проверка никому не нужна. Для релиза такое
поведение недопустимо, прерванная публикация оставила бы тег без артефакта.

Разделение попутно оказалось и содержательным: у двух файлов разные
источники секретов. `test` генерирует пару ключей на месте, `staging`
берёт постоянную из защищённого окружения — об этом дальше, в разделе 6.
