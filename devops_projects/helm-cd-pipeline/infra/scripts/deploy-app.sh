...
# set -a на время подключения файла: переменные из него нужны как
# переменные окружения, иначе helm их не увидит.
#
# Директива стоит отдельной строкой прямо над source не для красоты:
# в записи `set -a; source ...; set +a` она относилась бы к первой
# команде в строке, то есть к set -a, и SC1091 всё равно срабатывал бы.
set -a
# shellcheck disable=SC1090,SC1091  # путь известен только во время работы
