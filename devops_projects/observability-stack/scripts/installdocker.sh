...
#
# Скрипт идемпотентный: при повторном vagrant provision он видит уже
# установленный Docker и выходит, не трогая систему. Это важно, потому что
# provisioning запускается заново при каждом vagrant up остановленной машины.
#
# sudo здесь не нужен: Vagrant выполняет shell-провижининг от root.
