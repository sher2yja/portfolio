#!/bin/bash
# Вход воркера в Swarm. Вызывается Vagrantfile на worker01 и worker02.
#
# Порядок подъёма машин Vagrant не гарантирует, поэтому воркер не идёт
# на менеджер, а ждёт появления файла с токеном в общей папке — до 60 секунд.
# Проверяется и существование файла, и непустота: manager.sh создаёт файл
# перенаправлением вывода, так что пустой файл успевает появиться раньше,
# чем в него попадёт токен.
